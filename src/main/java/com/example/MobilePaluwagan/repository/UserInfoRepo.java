...
public interface UserInfoRepo extends JpaRepository<UserInfo, Long> {
    ...
    @Query(value = "SELECT ul.id, up.first_name, up.last_name, r.role_name, up.verified_date, ul.has_savings_account, ul.has_loan " +
            "FROM user_login ul " +
            "JOIN user_profile up ON ul.id = up.user_id " +
            "JOIN roles r ON r.id = ul.role_id " +
            "WHERE (CAST(:name AS text) IS NULL OR LOWER(up.first_name) LIKE LOWER(CONCAT('%', CAST(:name AS text), '%'))) " +
            "AND (CAST(:surname AS text) IS NULL OR LOWER(up.last_name) LIKE LOWER(CONCAT('%', CAST(:surname AS text), '%'))) " +
